...
def contarBash(rdd):
     ...
     for usuario in rdd:
	     ...
	     shell = usuario.split(":")[6]
             contadorShell += 1
     yield(shell,contadorShell)

# cargamos ficheros y trabajamos sobre cada particion de forma independiente(.mapPartitionsWithIndex) 
rddShell = sc.textFile("file:/home/training/julio/Mod14-Particiones/Ficheros/passwd*").mapPartitions(contarBash)

for registro in rddShell.collect():
     print registro
